...
class Importer:
    ...
    def import_publication_type_list(self):
        ...
        types_to_save = set()
        with open(
            "I://WORK//LEARN_CENTER//Python//python-to-middle//block_10//explain//task_1//books.csv",
            encoding="utf-8",
        ) as f:
        ...
        for row in sorted_list:
            list_for_save.append(PublicationType(name=row))
                ...
                if publication_type is not None:
                    if row["Аннотация"] is None:
                        if len(row["Вид издания"]) == 4:
                            book_card.publication_type = self.get_publication_type(
                                row["Название издания"]
                            )
                            book_card.name += row["Авторы"].replace(
                                symbol_to_replace, ""
                            )
                            book_card.publication_number = row["Вид издания"].replace(
                                symbol_to_replace, ""
                            )
                            book_card.page_number = self.get_page_number(
                                row["Год издания"]
                            )
                            book_card.publication_date = self.get_publication_date(
                                row["Кол-во стр."]
                            )
                        ...
                        if len(books_to_save) % 60 == 0:
                            rack = BookRack(name=len(racks_to_save) + 1, room=room)

                            racks_to_save.append(rack)

                        if len(books_to_save) % 10 == 0:
                            shelf = BookShelf(name=len(shelf_to_save) + 1, rack=rack)
                        ...
                        book_card = BookCard(
                            ...
                            publication_date=self.get_publication_date(
                                row["Срок окончания авторского договора"]
                            ),
                            description=row["Аннотация"].replace(symbol_to_replace, ""),
                            isbn=row["ISBN"],
                            book_shelf=BookShelf.objects.filter(
                                name=shelf.name
                            ).first(),
                        )
                    ...
                    if publication_name is not None:
                        ...
                        if row["Авторы"] is not None:
                            if book_card.description is None:
                                book_card.description = row["Авторы"].replace(
                                    symbol_to_replace, ""
                                )
                            else:
                                book_card.description += " " + row["Авторы"].replace(
    ...
    def add_empty_shelfs(self):
        ...
        for i in range(shelf_count_to_last):
            shelf_to_save.append(BookShelf(name=shelf_num + i, rack=last_rack))

        racks_to_save = []
        last_room = last_rack.room
        rack_num = 5 - rack_to_add_count + 1
        for i in range(rack_to_add_count):
            rack = BookRack(name=rack_num + i, room=last_room)
            racks_to_save.append(rack)

            for shelf_number in range(6):
                shelf_to_save.append(BookShelf(name=shelf_number + 1, rack=rack))
    ...
    def get_publication_date(self, pb_date_str):
        ...
        if len(pb_dat_list) != 3:
            ...
            try:
                date = datetime(
                    int(pb_dat_list[2]), int(pb_dat_list[1]), int(pb_dat_list[0])
                )
            except:
                year = pb_dat_list[2]
...
class ReportHelper:
    ...
    def get_book_count_in_library_by_author(cls, author):
        ...
        """
        book_id_list = list(
            BookIssueJournal.objects.filter(returned=False).values_list(
                "book", flat=True
            )
        )
        query = BookCard.objects.filter(authors__contains=author).filter(
            ~Q(id__in=book_id_list)
        )
    ...
    def get_popular_books_for_month(cls):
        ...
        query_popular_books = (
            BookCard.objects.annotate(
                period_book_read=Count(
                    "issue_journal", filter=q_start_date & q_end_date
                )
            )
            .filter(period_book_read__gt=0)
            .order_by("-period_book_read")
        )[:10]
    ...
    def get_popular_moving_books_for_month(cls):
        ...
        query_moving_books = (
            ...
            .filter(moving_count__gt=0)
            .order_by("-moving_count")
        )[:10]
    ...
    def get_active_readers(cls):
        ...
        result = query_popular_books.values(
            "reader__last_name", "reader__name", "book_count"
        )
    ...
    def get_overdue_returns(cls):
        ...
        result = (
            BookIssueJournal.objects.filter(returned & q_returned_date)
            .annotate(
                last_name=F("reader_card__reader__last_name"),
                name=F("reader_card__reader__name"),
            )
    ...
    def get_outside_library_readers(cls):
        ...
        result = (
            outside_library_readers.annotate(
                last_name=F("reader__last_name"), name=F("reader__name")
            )
    ...
    def get_read_pages_by_publication(cls):
        ...
        result = (
            BookIssueJournal.objects.filter(q_start_date & q_end_date)
